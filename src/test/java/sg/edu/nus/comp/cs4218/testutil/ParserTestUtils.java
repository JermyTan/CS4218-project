...
public final class ParserTestUtils {
    ...
    public static List<String> dropFirst(List<String> args, int count) {
        int start = Math.min(args.size(), Math.max(count, 0));
        return List.copyOf(args.subList(start, args.size()));
    }

    public static List<String> dropLast(List<String> args, int count) {
        int end = Math.max(0, args.size() - Math.max(count, 0));
